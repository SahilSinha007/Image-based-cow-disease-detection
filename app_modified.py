...
class_indices = None
class_names = None
# Lazy-loaded ImageNet model for cow-detection fallback
imagenet_model = None
model_performance = {
...
def preprocess_image_successful(img):
    ...
    try:
        ...
        # Also prepare a MobileNet-friendly copy for potential ImageNet checks
        try:
            mobilenet_array = image.img_to_array(img_resized.copy())
            mobilenet_array = np.expand_dims(mobilenet_array, axis=0)
            mobilenet_array = preprocess_input(mobilenet_array)
        except Exception:
            mobilenet_array = None
...
def load_imagenet_fallback():
    """Lazy-load MobileNetV2 (ImageNet) for cow presence checks."""
    global imagenet_model
    try:
        if imagenet_model is None:
            logger.info("📥 Loading MobileNetV2 (ImageNet) for cow-presence checks...")
            imagenet_model = MobileNetV2(weights='imagenet')
            logger.info("✅ MobileNetV2 loaded")
    except Exception as e:
        logger.warning(f"⚠️ Could not load ImageNet model: {e}")
        imagenet_model = None
    return imagenet_model


def is_likely_cow(processed_img_array, mobilenet_array, disease_confidence, predicted_class):
    """Decide whether an uploaded image likely contains a cow (or close-up of cow parts).

    Strategy:
      1. If the disease model is confident (>= DISEASE_CONF_THRESHOLD) we accept.
      2. Otherwise try an ImageNet MobileNetV2 check: decode top predictions and look for
         bovine-related labels (cow/ox/bull/zebu/calf/jersey/etc.). If found with
         reasonable probability, accept. If ImageNet model isn't available, fall back
         to stricter disease-confidence requirement.
    """
    DISEASE_CONF_THRESHOLD = 0.35  # 35% = allow close-ups that the disease model recognizes

    # We'll prefer ImageNet checks when available to avoid confidently accepting
    # non-cow images (documents/screens/game screenshots). If ImageNet is not
    # available, we'll fall back to disease-class acceptance below.

    # For robust detection, prefer running ImageNet checks if available.
    bovine_prob_threshold = 0.05   # modest probability to accept bovine detection
    non_natural_prob_threshold = 0.15  # stronger probability to reject non-natural/game/document

    try:
        im_model = load_imagenet_fallback()
        # prepare mobilenet_array if not provided
        if im_model is not None:
            if mobilenet_array is None and processed_img_array is not None:
                try:
                    mobilenet_array = preprocess_input((processed_img_array.astype('float32') * 255.0))
                except Exception:
                    mobilenet_array = None

            if mobilenet_array is not None:
                preds = im_model.predict(mobilenet_array)
                decoded = decode_predictions(preds, top=5)[0]
                logger.info(f"🔎 ImageNet top predictions for cow-presence: {decoded}")

                # 1) If ImageNet finds bovine-related labels with modest prob -> accept
                bovine_keywords = ['cow', 'ox', 'bull', 'zebu', 'calf', 'jersey', 'bison', 'cattle']
                for label, name, prob in decoded:
                    lname = name.lower()
                    for kw in bovine_keywords:
                        if kw in lname and float(prob) >= bovine_prob_threshold:
                            logger.info(f"✅ ImageNet detected bovine label '{name}' with prob {float(prob):.3f}")
                            return True, f'imagenet_{name}'

                # 2) If ImageNet finds document/screen/game/statue-like labels with higher prob -> reject
                non_natural_keywords = [
                    'paper', 'book', 'notebook', 'envelope', 'menu', 'newspaper', 'letter', 'file', 'binder',
                    'web_site', 'screen', 'monitor', 'menu', 'comic', 'comic_book', 'book_jacket', 'packet', 'menu',
                    'statue', 'sculpture', 'mask', 'armor', 'samurai', 'sword', 'toy', 'video_game', 'castle', 'screen'
                ]
                for label, name, prob in decoded:
                    lname = name.lower()
                    for kw in non_natural_keywords:
                        if kw in lname and float(prob) >= non_natural_prob_threshold:
                            logger.info(f"❌ ImageNet non-natural label '{name}' ({float(prob):.2f}) => rejecting")
                            return False, f'non_natural_{name}'

                # 3) Additional conservative rule: if the top ImageNet label has a
                # reasonable probability (>= 0.10) and it's not bovine or a natural
                # scene/animal, reject the image. This helps catch game screenshots,
                # statues, weapons, or other man-made scenes that the disease model
                # might mistakenly classify as 'healthy'.
                top_label, top_name, top_prob = decoded[0]
                top_name_l = top_name.lower()
                natural_scene_keywords = ['field', 'pasture', 'grass', 'ox', 'cow', 'bull', 'horse', 'goat', 'sheep', 'cattle', 'meadow', 'mountain', 'rock', 'stone', 'boulder', 'tree', 'forest', 'waterfall', 'stream', 'lake', 'river', 'pond', 'animal', 'dog', 'cat', 'bird']
                if float(top_prob) >= 0.10:
                    # if top label is not bovine and not a natural scene keyword, reject
                    is_bovine_top = any(kw in top_name_l for kw in bovine_keywords)
                    is_natural_top = any(kw in top_name_l for kw in natural_scene_keywords)
                    if not is_bovine_top and not is_natural_top:
                        logger.info(f"❌ Top ImageNet label '{top_name}' ({float(top_prob):.2f}) looks non-natural; rejecting")
                        return False, f'top_label_non_natural_{top_name}'

                # After ImageNet checks: if disease class predicted with confidence,
                # accept (ImageNet did not veto). This allows close-ups while still
                # letting ImageNet reject obvious non-cow images.
                disease_classes_allow = ['mastitis', 'lumpy_skin', 'foot_mouth']
                try:
                    if predicted_class in disease_classes_allow and disease_confidence >= DISEASE_CONF_THRESHOLD:
                        logger.info(f"✅ ImageNet did not veto and disease-class '{predicted_class}' is confident; accepting close-up")
                        return True, 'disease_class_confident'
                except Exception:
                    pass

    except Exception as e:
        logger.debug(f"⚠️ ImageNet cow-presence check failed or unavailable: {e}")

    # Fallback based on disease model confidence (if ImageNet not decisive)
    if disease_confidence >= DISEASE_CONF_THRESHOLD:
        logger.info(f"✅ Disease model confidence {disease_confidence:.2f} >= threshold {DISEASE_CONF_THRESHOLD}")
        return True, 'disease_confident'

    # Otherwise, reject
    logger.info("❌ Low disease confidence and ImageNet inconclusive; rejecting upload")
    return False, 'low_confidence_no_cow'

    # 2) Try ImageNet fallback to detect bovine presence
    try:
        im_model = load_imagenet_fallback()
        if im_model is None:
            logger.info("ℹ️ ImageNet fallback unavailable; blocking due to low disease confidence")
            return False, 'no_imagenet_and_low_confidence'

        # If no mobilenet_array provided, try to build one from processed_img_array
        if mobilenet_array is None and processed_img_array is not None:
            try:
                # processed_img_array expected in range [0,1] with shape (1,224,224,3)
                mobilenet_array = preprocess_input((processed_img_array.astype('float32') * 255.0))
            except Exception:
                mobilenet_array = None

        if mobilenet_array is None:
            logger.info("ℹ️ No MobileNet input available for ImageNet check")
            return False, 'no_mobilenet_input'

        preds = im_model.predict(mobilenet_array)
        decoded = decode_predictions(preds, top=5)[0]
        logger.info(f"🔎 ImageNet top predictions for cow-check: {decoded}")

        bovine_keywords = ['cow', 'ox', 'bull', 'zebu', 'calf', 'jersey', 'bison', 'cattle']
        # Check top decoded labels for bovine keywords
        for label, name, prob in decoded:
            lower = name.lower()
            for kw in bovine_keywords:
                if kw in lower:
                    # require a modest probability to accept (e.g., >= 0.05)
                    if prob >= 0.05:
                        logger.info(f"✅ ImageNet detected bovine label '{name}' with prob {prob:.3f}")
                        return True, f'imagenet_{name}'

        # Not detected by ImageNet -> reject
        logger.info("❌ ImageNet did not detect bovine-related labels; rejecting upload")
        return False, 'imagenet_no_bovine'

    except Exception as e:
        logger.warning(f"⚠️ Error during ImageNet bovine check: {e}")
        return False, 'imagenet_error'
...
def predict_successful():
    ...
    try:
        ...
        # Cow-presence validation: ensure the uploaded image is of a cow (or cow part).
        # Accept if disease model is reasonably confident OR ImageNet detects bovine labels.
        # Note: mobilenet_array was prepared in preprocessing; we stored it there.
        mobilenet_array = None
        try:
            # Try to re-create a MobileNet-ready array from the PIL image if available
            # (we still have 'img' in scope from earlier)
            mobilenet_array = preprocess_input(np.expand_dims(image.img_to_array(img.resize((224, 224)).convert('RGB')), axis=0))
        except Exception:
            mobilenet_array = None

        # Note: earlier preprocessing created a mobilenet copy, but to avoid breaking if absent, we'll use
        # a combined strategy: call is_likely_cow with available arrays below.

        # Prefer using mobilenet_array if present and call is_likely_cow properly
        try:
            if mobilenet_array is not None:
                is_cow, cow_reason = is_likely_cow(processed_img, mobilenet_array, confidence, predicted_class)
            else:
                # Call with a None mobilenet_array; function will try to load ImageNet and fail gracefully
                is_cow, cow_reason = is_likely_cow(processed_img, None, confidence, predicted_class)
        except Exception as e:
            logger.warning(f"⚠️ Cow-presence check raised an exception: {e}")
            is_cow, cow_reason = False, 'check_exception'

        if not is_cow:
            logger.info(f"🚫 Upload rejected: not recognized as cow (reason={cow_reason})")
            return jsonify({'error': 'Uploaded image does not appear to contain a cow or relevant cow body part. Please upload a clear image of a cow (full body or close-up of mouth, hoof, udder).', 'success': False}), 400
